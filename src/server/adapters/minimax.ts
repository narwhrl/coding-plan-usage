 ...
 * - data.model_remains[]，取 model_name === 'general' 行
  ...
  async fetchUsage(ctx): Promise<AdapterResult> {
    ...
    for (const base of bases) {
      for (const path of REMAINS_PATHS) {
        ...
        const session = laneWindow(general, "current_interval_remaining_percent", "current_interval_status", "end_time", "5h", "5h interval");
        const weekly = laneWindow(general, "current_weekly_remaining_percent", "current_weekly_status", "weekly_end_time", "weekly", "Weekly");
